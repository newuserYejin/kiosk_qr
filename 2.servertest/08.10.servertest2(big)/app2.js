...
connection.connect((err) => {
  ...
  const getMenuDataByRange = (menuRange, callback) => {
    const sql = 'SELECT menu_name, price, menu_explan FROM tb_menu WHERE menu_num BETWEEN ? AND ?';
...
// 라우트 설정
app.get('/BigOrder', (req, res) => {
    getMenuData((menuData) => {
        res.render('BigOrder', { menuData }); // menuData 변수를 템플릿으로 넘겨줌
    });
});
